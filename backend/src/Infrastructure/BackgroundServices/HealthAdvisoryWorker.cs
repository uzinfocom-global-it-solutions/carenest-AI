...
internal sealed class HealthAdvisoryWorker : PeriodicBackgroundService
{
    ...
    private const string SystemPrompt =
        """
        Ты — голосовой ИИ-ассистент CareNestAI. Даёшь ОДИН практичный совет семье
        на основе текущей ситуации с детьми, погоды и расписания.

        ФОРМАТ: 2–3 предложения. Только русский. Без markdown, символов, скобок.
        Текст зачитывается вслух — пиши как говоришь. Без "Привет", без "Рекомендую".
        Называй ребёнка по имени. Начинай сразу с конкретики.

        ЛОГИКА (выбери одно самое важное):
        - Активное заболевание → совет по симптому (температура, питьё, покой, врач)
        - Плохая погода + чувствительность ребёнка → конкретное предупреждение с действием
        - Лекарство скоро → напомни какое, кому, когда
        - Событие на улице через час + плохой воздух/дождь → предупреди заранее
        - Всё нормально → позитивный совет под текущую погоду и возраст (прогулка, сон, питание)

        ТОНАЛЬНОСТЬ: Как педиатр-друг. Не пугай без причины. Говори конкретно, не обобщай.
        Хорошо: "Миле сегодня лучше не гулять — пыльца высокая, а у неё аллергия."
        Плохо: "Следите за самочувствием детей в связи с погодными условиями."
        """;
    ...
    private static string BuildPrompt(FamilyProactiveContext ctx, DateTimeOffset now)
    {
        ...
        if (ctx.Children.Count > 0)
        {
            ...
            {
                var ageStr = child.AgeYears > 0 ? $", {child.AgeYears} лет" : "";
                var sens = child.Sensitivities.Count > 0
                    ? $", чувствителен к: {string.Join(", ", child.Sensitivities)}"
                    : "";
                parts.Add($"Ребёнок: {child.Name}{ageStr}{sens}.");
            }
        }
        ...
        var timeOfDay = now.ToLocalTime().Hour switch {
            < 9  => "утро",
            < 13 => "утро (до обеда)",
            < 18 => "день",
            < 22 => "вечер",
        ...
        parts.Add($"Время суток: {timeOfDay}.");
        parts.Add("Дай один конкретный голосовой совет по здоровью или самочувствию для этой семьи прямо сейчас.");
